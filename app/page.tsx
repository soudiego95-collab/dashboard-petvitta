        ...
        <div className="mx-auto max-w-[1540px] px-5 py-7 sm:px-8 lg:px-10 lg:py-9">
          ...
          {isFiltering ? (
            ...
            <>
              ...
              {page === 'Filiais' && <Branches selectedBranch={selectedBranch} setSelectedBranch={setSelectedBranch} filteredBranches={filteredBranches} filteredMonths={filteredMonths} />}
              {page === 'Receitas' && <Revenue scale={scale} />}
              {page === 'Custos' && <Costs scale={scale} />}
              {page === 'Investimentos' && <Investments scale={scale} filteredMonths={filteredMonths} />}
...
function Executive({ current, previous, growthDetail, growthCaption, marginDelta, marginCaption, filteredMonths, filteredBranches, branchesEmAtencao, totalReceita, totalResultado, margemOperacional, setPage }: any) {
  ...
  return <>
  <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Kpi label="Receita líquida" value={brl(totalReceita)} detail={pct(growthDetail)} positive={growthDetail >= 0} icon={CircleDollarSign} caption={growthCaption} /><Kpi label="Resultado operacional" value={brl(totalResultado)} detail={pct(growthDetail)} positive={growthDetail >= 0} icon={growthDetail >= 0 ? TrendingUp : TrendingDown} tone={growthDetail >= 0 ? 'teal' : 'red'} caption={growthCaption} /><Kpi label="Margem operacional" value={`${margemOperacional.toFixed(1).replace('.', ',')}%`} detail={`${marginDelta >= 0 ? '+' : ''}${marginDelta.toFixed(1).replace('.', ',')} p.p.`} positive={marginDelta >= 0} icon={Activity} tone={marginDelta >= 0 ? 'teal' : 'amber'} caption={marginCaption} /><Kpi label="Filiais em atenção" value={`${branchesEmAtencao.length} de ${filteredBranches.length}`} detail={atencaoLabel} positive={branchesEmAtencao.length === 0} icon={Building2} onClick={() => setPage('Filiais')} /></div>
...
function Costs({ scale = 1 }: { scale?: number }) { return <><div className="grid gap-4 sm:grid-cols-3"><Kpi label="Custos variáveis / receita" value="17,6%" detail="+1,8 p.p." positive={false} icon={WalletCards} tone="amber"/><Kpi label="Despesas / receita" value="45,2%" detail="+2,4 p.p." positive={false} icon={CircleDollarSign} tone="red"/><Kpi label="Concentração top 5" value="68,4%" detail="+5,3 p.p." positive={false} icon={BarChart3} tone="amber"/></div><section className="mt-5 rounded-2xl border border-[#dfebe7] bg-white p-5 sm:p-6"><div className="mb-5"><h3 className="font-bold">Ofensores de custo</h3><p className="mt-1 text-xs text-[#8ba09a]">Ranking por maior aumento absoluto no período · valores ajustados ao filtro ativo</p></div><div className="overflow-x-auto"><table className="w-full min-w-[650px] text-left text-xs"><thead className="border-y border-[#edf3f0] text-[10px] uppercase tracking-wider text-[#9aaca8]"><tr>{['Componente','Classificação','Valor atual','Variação','Participação'].map(x=><th key={x} className="px-3 py-3">{x}</th>)}</tr></thead><tbody className="divide-y divide-[#edf3f0]">{costs.map(c=><tr key={c.name}><td className="px-3 py-4 font-bold">{c.name}</td><td className="px-3 py-4 text-[#718a84]">{c.type}</td><td className="px-3 py-4 font-semibold">{brl(c.value*scale)}</td><td className={`px-3 py-4 font-bold ${c.delta>0?'text-[#c26156]':'text-[#248a61]'}`}>{pct(c.delta)}</td><td className="px-3 py-4 text-[#718a84]">{(c.value/1282687*100).toFixed(1).replace('.',',')}%</td></tr>)}</tbody></table></div></section></> }
function Investments({ scale = 1, filteredMonths = months }: { scale?: number; filteredMonths?: typeof months }) { return <><div className="grid gap-4 sm:grid-cols-3"><Kpi label="Investimento acumulado" value={brl(728119*scale)} detail="25,6% da receita" icon={TrendingUp} tone="amber"/><Kpi label="Investimento no mês" value={brl(18216*scale)} detail="-21,2%" positive={false} icon={CircleDollarSign}/><Kpi label="Principal categoria" value="Equipamentos" detail="42,8% do CAPEX" icon={Package}/></div><section className="mt-5 rounded-2xl border border-[#dfebe7] bg-white p-6"><h3 className="font-bold">Investimentos por período</h3><p className="mt-1 text-xs text-[#8ba09a]">Mantidos fora do resultado operacional</p><div className="mt-6 h-[260px]"><ResponsiveContainer><BarChart data={filteredMonths}><CartesianGrid vertical={false} stroke="#edf3f0"/><XAxis dataKey="month" tickLine={false} axisLine={false} tick={{fontSize:10,fill:'#8aa09c'}}/><YAxis tickLine={false} axisLine={false} tick={{fontSize:10,fill:'#8aa09c'}} tickFormatter={v=>`${v/1000}k`}/><Tooltip formatter={(v:any)=>brl(v)}/><Bar dataKey="receita" name="Investimentos" fill="#d0a55b" radius={[4,4,0,0]} /></BarChart></ResponsiveContainer></div></section></> }
